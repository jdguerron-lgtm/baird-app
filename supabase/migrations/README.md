...
| `20260709_renombrar_estados_contract.sql` | aplicada (2026-07-09, deploy 8060df3 verificado READY antes) | Barre strays escritos por el código viejo durante la ventana de deploy y cierra el CHECK solo a los nombres nuevos |
| `20260709_purga_estados_contract.sql` | **aplicar DESPUÉS de verificar el deploy de la purga** (el UPDATE de filas `diagnostico_pendiente→asignada` se corre ANTES del deploy — es seguro con el código viejo) | Purga y fusión 22→18: `diagnostico_pendiente` fusionado en `asignada` (es_garantia distingue el flujo); eliminados los muertos `pendiente`, `cotizacion_aprobada` (aprobar salta directo a en_proceso/esperando_repuesto) y `reagendamiento_pendiente` (reagendar conserva estado). CHECK final = 18 estados = `ESTADOS_VALIDOS` |
| `20260709_supervisor_codigo_acceso.sql` | aplicada vía MCP (2026-07-09) | Entrada de autoservicio `/supervisor` con OTP por WhatsApp: columnas `codigo_acceso_hash` (sha256, nunca el código en claro), `codigo_acceso_expira_at` (10 min), `codigo_acceso_intentos` (máx 5) y `codigo_acceso_enviado_at` (cooldown 60s) en `supervisores`. Plantilla `supervisor_codigo_v1` (AUTHENTICATION, APPROVED). Ver `docs/SEGURIDAD.md` § "Entrada de autoservicio con OTP" |
| `20260802_repuesto_en_camino_guia.sql` | aplicada vía MCP (2026-08-02) | Flujo de repuestos v2: estado nuevo `repuesto_en_camino` (CHECK 18→19), columnas `guia_envio_url/numero/at/por` en `solicitudes_servicio` (guía de envío subida por el supervisor desde su portal) y `cantidad` en `repuestos_pendientes` (requerimiento exacto al supervisor). Garantía + esperar_repuesto pasa DIRECTO del diagnóstico a `esperando_repuesto` (se elimina el gate `pendiente_pricing`/tiempo de entrega). Ver `docs/MAQUINA-DE-ESTADOS.md` |
| `20260824_tecnicos_arl.sql` | aplicada vía MCP (2026-08-24) — verificada con INSERT/DELETE de prueba vía PostgREST | Pregunta "¿Tienes ARL?" en el registro de técnicos: columna `tiene_arl BOOLEAN` **nullable** en `tecnicos` (NULL = no informado, los 16 técnicos previos quedan así; el formulario nuevo siempre manda true/false). Incluye `NOTIFY pgrst, 'reload schema'` para evitar el PGRST204 del schema cache documentado en `20260701`. Aditiva e idempotente. Rollback: `ALTER TABLE public.tecnicos DROP COLUMN tiene_arl;` |
| `20260825_repuesto_recordatorios.sql` | aplicada vía MCP (2026-08-25) | Recordatorios del ciclo de repuesto: tipo `recordatorio_repuesto` en el CHECK de `solicitud_eventos` (14→15) + columnas `repuesto_recordatorio_count`/`repuesto_recordatorio_at` en `solicitudes_servicio` (tracking del cron `/api/cron/repuesto-recordatorio`, patrón de `horario_recordatorio_*` de `20260807`). Aditiva e idempotente. Si no se aplica, solo el cron falla (el endpoint `/api/supervisor/repuesto-entregado` y el resto del flujo no usan las columnas). Rollback: `ALTER TABLE solicitudes_servicio DROP COLUMN repuesto_recordatorio_count, DROP COLUMN repuesto_recordatorio_at;` + re-crear el CHECK sin el tipo nuevo |
