...
- **Subida de evidencias — compat con Android de bajo costo** (2026-05-14): el flujo de upload del técnico (`/tecnico/{token}/diagnostico/{id}` y `/tecnico/{token}/completar/{id}`) usa 3 helpers compartidos para sobrevivir a WebView de WhatsApp/Instagram, MIME vacío en Samsung/Mi/Huawei, y cámaras móviles modernas que producen JPEG >10MB:
  - `src/lib/utils/file-validation.ts` — `classifyMedia(file)` y `inferContentType(file)`. Si `file.type` viene vacío (algunos WebView Android), cae a detección por **extensión** del nombre. **Estrictamente más permisivo** que `file.type.startsWith('image/')` — todo lo que pasaba antes sigue pasando.
  - `src/lib/utils/image-compress.ts` — `compressImage(file)` comprime JPEG a 2048px @ 0.85. Wrapped en try/catch — si algo falla (HEIC no decodificable, canvas no disponible, etc.) devuelve **el archivo original sin tocar**. Solo afecta a imágenes >1.5MB. En Safari iOS de paso convierte HEIC → JPEG.
  - `src/components/ui/InAppBrowserBanner.tsx` — banner dismissible que solo aparece si UA matchea `WhatsApp|Instagram|FBAN|FBAV|FB_IAB|Line\/|MicroMessenger`. iPhone Safari, Edge, Chrome Android, Samsung Internet, Brave **NO lo ven**. Usa `useSyncExternalStore` para evitar hydration mismatch sin disparar `react-hooks/set-state-in-effect`.
  - **Límite de tamaño:** 25MB raw en cliente (antes 10MB en diagnóstico, 5MB en completar). El bucket no tiene `file_size_limit`. Después de comprimir el archivo real subido suele ser <2MB.
  - **Retry de upload (2026-05-15):** cada foto se reintenta hasta 3 veces (0ms, 500ms, 1500ms) usando un path único por intento (`_r1`, `_r2`) para no chocar con `upsert: false` si el blob ya quedó server-side. Sobrevive a 3G/HSPA flaky.
  - **Feedback granular (2026-05-15):** si la subida es parcial (subió X de Y) el técnico ve un aviso ámbar en la pantalla de éxito en vez de pasar silenciosamente. Estado: `subidaParcial` en ambas páginas.
  - **Cleanup de blob URLs (2026-05-15):** `useEffect` con `previewsRef` revoca los `URL.createObjectURL` al desmontar. Antes en Android gama baja se acumulaban ~32MB y crasheaba la pestaña.
  - **Reglas al modificar este flujo:** mantener todo aditivo. La compresión NUNCA debe bloquear una subida que antes funcionaba (try/catch + fallback). El banner es informativo, jamás bloqueante. Cualquier cambio que estreche el filtro de MIME debe testearse con UAs de Samsung Internet, Mi Browser y los 4 in-app browsers. El retry debe usar paths únicos por intento.
