...
**Particular (v2)** no pasa por admin gate en `reparar`. El técnico ingresa su `costoTecnico` (lo que quiere ganar: mano de obra + repuestos, estos con su 15% de descuento de la tienda) en el formulario de diagnóstico, y `/api/diagnostico` calcula automáticamente el total al cliente con la fórmula `costoTecnico × 1.13 utilidad Baird × (1 + mitad comisión Wompi) × 1.19 IVA` (≈ × 1.3675 desde 2026-08-19; × 1.3447 entre 2026-07-05 y 2026-08-18). La cotización se envía al cliente inmediatamente. Ver `docs/TARIFAS.md` § "Particular".
